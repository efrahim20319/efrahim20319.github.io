async function main() {
    ...
    const btnAllProducts  = document.querySelector("#addAll")
    const btnHideAllProducts  = document.querySelector("#hideAll")



    btnHideAllProducts.addEventListener("click", function () {
        const allDescs = document.querySelectorAll(".produto__description")
    ...
    inputPesquisar.addEventListener("input", function () {
        const value = this.value;
        const filteredProducts = produtos.filter(product => product.title.includes(value) || product.description.includes(value))
        if (filteredProducts.length == 0) {
            carregarProdutos(produtos)
        } else {
            carregarProdutos(filteredProducts);
    ...
    form.addEventListener("submit", function (evt) {
        ...
        .then(req => {
            ...
            const spanAddress = document.querySelector("#AddressReference")
            spanAddress.textContent = res.address
